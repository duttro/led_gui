...
class App(Tkinter.Tk):
    ...
    def __init__(self, parent, serThread1):
        self.parent = parent
    ...
    # make the top right close button minimize (iconify) the main window
        self.parent.protocol("WM_DELETE_WINDOW", self.parent.iconify)

    # make Esc exit the program
        Serial.t_stop.set()
        self.parent.bind('<Escape>', lambda e: self.serThread1.stop())
...
def main():
    # start the queue thread
        ...
        serThread1 = threading.Thread(target=Serial, args=(tx_queue, t_stop) )
        serThread1.start()

        root = Tkinter.Tk()
        app = App(root,serThread1)
