...
## 112814 added LED_make_DAT_file to create a data file for each buttons gif
## 112814 added function send_byte_matrix_as_asc_rgb to transmit byte array
## 112914 load_GIF needs to be modified to load the .dat file into matrix
# Create the queue
...
##        txt['font'] = ('consolas', '12')
    ...
    def cb_Store0(self):
        ...
        self.LED_make_DAT_file(filename,Matrix)
    ...
    def make_GIF(self,filename):
        ...
        #####self.send_asc_matrix(gifMatrix) , replaced a few line down
        #concatenate and create a byte array version of matrix
        m=[]
        for i in gifMatrix:
          m=m+i
          tempBuffer = bytearray('F'.encode("hex").decode("hex") + ''.join(m).replace('#','').decode("hex"))
        self.send_byte_matrix_as_asc_rgb(tempBuffer)
        return [filename,tempBuffer]
    
    def LED_make_DAT_file(self,filename,matrix):  
    ...
    def load_GIF(self,file_path):
        ...
        #fdat = open(file_path,"r+)
        #fdat.readlines()pix_val = fdat.read(1);
        
        file_name_parts = file_path.split(".");
        print"loading data file";
        print join(file_name_parts[1],'dat');
        with open(join(file_name_parts[1],'dat'),'r+') as fdat:
         gif_data = fdat.readlines(1)  #gif_data = [x.strip('\n') for x in f.readlines()]
         print gif_data; 
        
        #gif_data = tuple(open(file_path, 'r'))
        
        # convert the .gif into a matrix representing the 8 x 8 RGB grid
        #for s in self.box_coord:
        #  r,g,b = rgb_pix.getpixel((s[3]-4,s[2]-4)) # center of each box
        #  #print"load gif  coord = %16s X=%02i, Y=%02i  Matrix= #%02X%02X%02X" % (str(s), s[3]-4, s[2]-4, r,g,b )
        #  hexstring = ('#%02X%02X%02X'%(r,g,b))
        ...
        #self.send_asc_matrix(gifMatrix)
        ##
        #create and return a byte array version of matrix
        ##
        #m=[]
        #for i in gifMatrix:
        #  m=m+i
    ...
    def send_byte_matrix_as_asc_rgb(self,matrix):
        # matrix is a buffer length 194
        for s in matrix[::3]:  # [start:stop:step]
          x = s % 3  # % is mod
          y = s % 12
          r = int(s+0)
          g = int(s+1)
