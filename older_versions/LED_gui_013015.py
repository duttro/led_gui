...
##        txt['font'] = ('consolas', '12')
    ...
    def cb_clear_colors(self):
        self.bigMatrix = self.clear_colorArray()
        self.update_led_buttons(self.bigMatrix)
    ...
    def cb_Store0(self):
        ...
        self.b0= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
    ...
    def cb_Load0(self):
        ...
        self.button_image0.configure(image = self.image0)
        self.b0= buffer(Matrix,0,192)
        self.update_led_buttons(Matrix)
    ...
    def cb_Load1(self):
        ...
        self.button_image1.configure(image = self.image1)
        self.b1= buffer(Matrix,0,192)
        send_array_as_bin(Matrix)
    ...
    def make_GIF_fromBigMatrix(self,filename):
        ...
        for s in self.box_coord:
          ...
          tempBuffer.append(g)
          tempBuffer.append(b)
        self.update_led_buttons(tempBuffer)
    ...
    def send_array_as_bin(self,array):
        # array is a buffer length 192 = 8x * 8y * 3 bytes
        print("The array length is: %i"%( len(array)))
        
        self.q.put('Fp')  # send all cmd
        self.q.put(array) # send array
        for s in array:  # [start:stop:step]
          print ', '.join(array)
