...
##        txt['font'] = ('consolas', '12')
    ...
    def cb_clear_colors(self):
        self.bigMatrix = self.clear_colorArray()
        self.update_BIGmatrix_buttons(self.bigMatrix)
    ...
    def cb_Store0(self):
        ...
        self.b0= bytearray(Matrix) # now load the new image into the drawing pallet buffer
    ...
    def cb_Store1(self):
        ...
        self.b1= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store2(self):
        ...
        self.b2= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store3(self):
        ...
        self.b3= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store4(self):
        ...
        self.b4= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store5(self):
        ...
        self.b5= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store6(self):
        ...
        self.b6= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Store7(self):
        ...
        self.b7= buffer(Matrix,0,192) # now load the new image into the drawing pallet buffer
        self.update_file_LED_DAT(filename,Matrix)
    ...
    def cb_Load0(self):
        ...
        [filename,colorsMatrix,Matrix] = self.load_GIF_return_as_array()
        self.image0 = Tkinter.PhotoImage(file = filename)   
        self.button_image0.configure(image = self.image0)
        self.b0= bytearray(Matrix)
    ...
    def make_GIF_fromBigMatrix(self,filename):
        ...
        # fill in the 10x10 matrix
        for s in self.box_coord:
          #print"coord = %16s X=%02i, Y=%02i  bigMatrix= %s" % (str(s), (s[3]/12)-1, (s[2]/12)-1, str(self.Matrix[(s[3]/12)-1][(s[2]/12)-1]) )
          draw.rectangle( s , fill=self.bigMatrix[(s[3]/12)-1][(s[2]/12)-1] )
        # save the gif file
        img.save(filename, 'GIF', transparency=0)
        
        # now reload the image and convert to RGB values
        ...
        for s in self.box_coord:
          ...
          tempBuffer.append(g)
          tempBuffer.append(b)
        self.update_BIGmatrix_buttons(tempBuffer)
        self.send_array_as_asc_each_pixel_rgb(tempBuffer)
    ...
    def get_bytes_from_file(self,filename):  
        ...
        try:
          ...
          print"get_bytes_from_file: FOUND DAT FILE"
          for line in f:
        ...
        # if the .dat file doesn't exist use .gif file as source 
          ...
          #print tempBuffer
          return [ledColorMatrix,tempBuffer]
    ...
    def load_GIF_return_as_array(self):
        ...
        [ledColorMatrix,tempBuffer] = self.get_bytes_from_file(data_file_name)
        #print"tempBuffer"
        #for i in tempBuffer:
        #  print i
        return [self.file_path,ledColorMatrix,tempBuffer]
